...
class YouTubeAPI:
    ...
    def _post(self, endpoint, params, body):
        ...
        response = requests.post(f'{API_BASE}/{endpoint}', headers=headers, params=params, json=body, timeout=15)

        if response.status_code == 401:
            raise YouTubeAPIError('Session expired. Please sign in again.')
        if response.status_code == 403:
            raise YouTubeAPIError('YouTube API access denied. Your sign-in may not have permission to make '
                                   'changes yet - try signing out and signing in again.')
    ...
    def get_all_playlist_items(self, playlist_id, max_items=500):
        """Fetch every video in a playlist (paginating automatically), used for
        searching within a playlist. Capped at max_items to protect API quota
        on very large playlists."""
        all_videos = []
        page_token = None

        while True:
            result = self.get_playlist_items(playlist_id, page_token=page_token)
            all_videos.extend(result['videos'])

            page_token = result.get('next_page_token')
            if not page_token or len(all_videos) >= max_items:
                break

        return all_videos[:max_items]

    def get_all_my_playlists(self, max_items=200):
        """Fetch every one of the signed-in user's playlists, used for the
        'add video to playlist' picker."""
        all_playlists = []
        page_token = None

        while True:
            result = self.get_my_playlists(page_token=page_token)
            all_playlists.extend(result['playlists'])

            page_token = result.get('next_page_token')
            if not page_token or len(all_playlists) >= max_items:
                break

        return all_playlists[:max_items]

    def search_videos(self, query, page_token=None):
        params = {
            'part': 'snippet',
            'q': query,
            'type': 'video',
    ...
    def create_playlist(self, title, description='', privacy_status='private'):
        ...
        data = self._post('playlists', {'part': 'snippet,status'}, body)
        return data.get('id')
    ...
    def add_video_to_playlist(self, playlist_id, video_id):
        ...
        self._post('playlistItems', {'part': 'snippet'}, body)
