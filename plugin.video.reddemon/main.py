...
def add_video_item(video, is_folder=False):
    """Render a single video as a directory item, with an 'Add to Playlist'
    context menu option available via long-press/right-click."""
    list_item = xbmcgui.ListItem(label=video['title'])
    list_item.setArt({'thumb': video['thumbnail'], 'icon': 'DefaultVideo.png'})
    list_item.setInfo('video', {
        'title': video['title'],
        'plot': video['description'],
        'studio': video.get('channel', '')
    })
    list_item.setProperty('IsPlayable', 'true')

    add_to_playlist_url = get_url(action='add_to_playlist', video_id=video['video_id'])
    list_item.addContextMenuItems([
        ('Add to Playlist', f'RunPlugin({add_to_playlist_url})')
    ])
...
def show_main_menu():
    ...
    if config.is_signed_in():
        menu_items = [
            ...
            ('Create New Playlist', 'create_playlist'),
            ('Sign Out', 'signout'),
...
def show_playlist_items(playlist_id, page_token=None):
    ...
    if not page_token:
        search_item = xbmcgui.ListItem(label='Search This Playlist...')
        search_item.setArt({'icon': 'DefaultAddonsSearch.png'})
        url = get_url(action='search_playlist', playlist_id=playlist_id)
        xbmcplugin.addDirectoryItem(handle=handle, url=url, listitem=search_item, isFolder=True)

    for video in videos:
        add_video_item(video)
...
def search_playlist(playlist_id):
    """Search within a single (possibly huge) playlist for a matching title"""
    query = xbmcgui.Dialog().input('Search this playlist for...')
    ...
    progress = xbmcgui.DialogProgress()
    progress.create('Reddemon', f'Searching for "{query}"...')

    try:
        all_videos = api.get_all_playlist_items(playlist_id)
    except (AuthError, YouTubeAPIError) as e:
        progress.close()
        show_error(str(e))
        xbmcplugin.endOfDirectory(handle, succeeded=False)
    ...
    matches = [v for v in all_videos if query.lower() in v['title'].lower()]

    if not matches:
        show_error(f'No videos found matching "{query}" in this playlist.')
...
def search_youtube(query=None, page_token=None):
    if query is None:
        query = xbmcgui.Dialog().input('Search YouTube for...')
        if not query:
            xbmcplugin.endOfDirectory(handle, succeeded=False)
            return

    try:
        result = api.search_videos(query, page_token=page_token)
    except (AuthError, YouTubeAPIError) as e:
        show_error(str(e))
        xbmcplugin.endOfDirectory(handle, succeeded=False)
        return
    ...
    if result.get('next_page_token'):
        next_item = xbmcgui.ListItem(label='Next Page >>')
        next_item.setArt({'icon': 'DefaultFolder.png'})
        url = get_url(action='search_youtube', query=query, page_token=result['next_page_token'])
        xbmcplugin.addDirectoryItem(handle=handle, url=url, listitem=next_item, isFolder=True)
...
def create_playlist_prompt():
    title = xbmcgui.Dialog().input('New playlist name')
    ...
    choice = xbmcgui.Dialog().select('Who can see this playlist?', privacy_options)

    if choice == -1:
        choice = 0

    privacy_status = privacy_options[choice].lower()

    try:
        playlist_id = api.create_playlist(title, privacy_status=privacy_status)
    except (AuthError, YouTubeAPIError) as e:
        show_error(str(e))
        xbmcplugin.endOfDirectory(handle, succeeded=False, updateListing=False, cacheToDisc=False)
        return
    ...
    if playlist_id:
        xbmcgui.Dialog().notification('Reddemon', f'Playlist "{title}" created', xbmcgui.NOTIFICATION_INFO, 3000)
    else:
        show_error('Something went wrong creating the playlist.')

    xbmcplugin.endOfDirectory(handle, succeeded=False, updateListing=False, cacheToDisc=False)


def add_video_to_playlist_flow(video_id):
    try:
        playlists = api.get_all_my_playlists()
    except (AuthError, YouTubeAPIError) as e:
        show_error(str(e))
        xbmcplugin.endOfDirectory(handle, succeeded=False, updateListing=False, cacheToDisc=False)
        return
    ...
    options = [p['title'] for p in playlists] + [CREATE_NEW]

    choice = xbmcgui.Dialog().select('Add to which playlist?', options)
    ...
    try:
        if choice == len(playlists):
            ...
            playlist_title = playlists[choice]['title']

        api.add_video_to_playlist(playlist_id, video_id)
        xbmcgui.Dialog().notification('Reddemon', f'Added to "{playlist_title}"', xbmcgui.NOTIFICATION_INFO, 3000)
    except (AuthError, YouTubeAPIError) as e:
        show_error(str(e))
...
def router():
    ...
    if action is None:
        ...
        search_playlist(params.get('playlist_id'))
    elif action == 'search_youtube':
        search_youtube(query=params.get('query'), page_token=params.get('page_token'))
    elif action == 'create_playlist':
        create_playlist_prompt()
    elif action == 'add_to_playlist':
        add_video_to_playlist_flow(params.get('video_id'))
    else:
        show_main_menu()
