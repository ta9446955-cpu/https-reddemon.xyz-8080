...
// Uses a hardcoded channel ID — no OAuth needed
...
async function getChannelPlaylists() {
    ...
    const url = `https://www.googleapis.com/youtube/v3/playlists?part=snippet&channelId=${CHANNEL_ID}&maxResults=50&key=${YOUTUBE_API_KEY}`;
    const res = await fetch(url);
    if (!res.ok) throw new Error(`YouTube API error: ${res.status}`);
    const data = await res.json();
    cachedPlaylists = (data.items || []).map(item => ({
        id: item.id,
...
app.get('/manifest.json', async (req, res) => {
    try {
        ...
        res.json({
            ...
            description: 'YouTube playlists in Stremio.',
...
<body>
  ...
  <p>Watch YouTube playlists in Stremio.</p>
...
app.get('/health', (req, res) => {
    res.json({ status: 'ok', youtube: YOUTUBE_API_KEY ? 'set' : 'NOT SET' });
});
