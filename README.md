...
| 📦 **Addon zip** (v1.0.6, install directly, no auto-updates) | [plugin.video.reddemon-1.0.6.zip](https://raw.githubusercontent.com/ta9446955-cpu/https-reddemon.xyz-8080/main/plugin.video.reddemon/plugin.video.reddemon-1.0.6.zip) |
...
2. **Go to**: Kodi > Settings > Add-ons > Install from zip file
3. **Select** the downloaded `plugin.video.reddemon-1.0.6.zip`
4. **Restart** Kodi
...
If you'd rather use your own credentials (for example, if you're running into Google API quota limits from heavy shared use), Reddemon will use yours instead if you set them - it checks, in order: your own credentials entered in Reddemon's Settings, then the official YouTube addon's own API keys (Settings > API), then falls back to the built-in default.
